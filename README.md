...
> Status: **P1–P3 shipped** — point-in-time scan, continuous agent + `ClusterReadiness` CRD + server (SQLite/Postgres), fleet rollups, per-team scores, CI gate endpoint + GitHub Action, auditor CSV/HTML export, per-cluster tokens. P4 (web dashboard + community registry pipeline) in development. Design: `docs/superpowers/specs/2026-06-10-upgradescope-design.md`.
